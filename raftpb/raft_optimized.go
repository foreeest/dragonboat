...
func (m *MessageBatch) Unmarshal(dAtA []byte, bitset uint64) error {
	var to_list []uint64
	for i := 0; i < 32; i++ {
		var to_i uint64
		to_i = bitset & (1 << (i + 32))
		if to_i > 0 {
			to_list = append(to_list, uint64(i))
		}

	}
	//fmt.Printf("bitset: %d\n",bitset)
	//debug only JPF want to delete at 7/28 down
	if len(to_list) < 1 {

		fmt.Printf("bitset: %d\n", uint32(bitset>>32)) //输出高32位
		for j := 0; j < 32; j++ {
			var to_i_print uint64
			to_i_print = bitset & (1 << (j + 32))
			fmt.Printf("%d  ", to_i_print)
		}
		fmt.Printf("in func(m *MessageBatch) Unmarshal(dAtA []byte, bitset uint64):to_list's len<1\n")
	}
	//up
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		...
		if fieldNum <= 0 {
			return fmt.Errorf("MessageBatch: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
			...
			if err := m.Requests[len(m.Requests)-1].Unmarshal(dAtA[iNdEx:postIndex], to_list); err != nil {
				return err
			}
			iNdEx = postIndex
		case 2:
...
// Unmarshal unmarshals the message instance using the input byte slice.
func (m *MY_Message) Unmarshal(dAtA []byte, to_list []uint64) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		if fieldNum <= 0 {
			return fmt.Errorf("rproto: Message: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
		...
		case 2:
			...
			//JPF delete down at 7/28
			// m.To = append(m.To[:0], make([]uint64, 1)...) // 初始化 To 切片，确保有至少一个元素
			// m.To[0] = 1
			//JPF delete up at 7/28
			m.To = to_list
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowRaft
				...
				b := dAtA[iNdEx]
				iNdEx++
				//m.To[0] |= (uint64(b) & 0x7F) << shift
				if b < 0x80 {
					break
				}
				fmt.Printf("what? didn't out? to[0]:%d\n", m.To[0])
			}
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field From", wireType)
...
func (m *MY_Message) SizeUpperLimit() int {
	...
	l += 1 + sovRaft(m.To[0]) // From
	//JPF want to edit at 7/28 up
